...
export function getRuleEditorHtml(rule: ArchitecturalRule): string {
  const constraints = Object.values(ConstraintType)
    .map((c) => `<option value="${escapeHtml(c)}" ${rule.constraint === c ? 'selected' : ''}>${escapeHtml(c)}</option>`)
    .join('\n');

  const severities = Object.values(Severity)
    .map((s) => `<option value="${escapeHtml(s)}" ${rule.severity === s ? 'selected' : ''}>${escapeHtml(s)}</option>`)
    .join('\n');
