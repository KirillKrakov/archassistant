  ...
  private async loadMetrics(): Promise<void> {
    try {
      ...
      if (!metrics && !comparison) {
        throw new Error('Failed to load metrics and strategy comparison');
      }
      ...
      this.panel.webview.postMessage({
        ...
        comparison,
        errors: [
          metricsResult.status === 'rejected' ? metricsResult.reason?.message || 'Metrics failed' : null,
          comparisonResult.status === 'rejected' ? comparisonResult.reason?.message || 'Comparison failed' : null
        ].filter(Boolean)
      });
    } catch (error: any) {
...
<head>
  ...
  <style>
    ...
    h2, h3 { margin-top: 0; }
    .section { margin-bottom: 24px; padding: 16px; background: var(--vscode-textBlockBackground); border-radius: 6px; }
    ...
    .error { color: var(--vscode-problemsErrorIcon); }
    .warning { color: var(--vscode-problemsWarningIcon); }
    button {
      padding: 8px 14px;
      border: none;
      border-radius: 4px;
  ...
  <script>
    ...
    window.addEventListener('message', event => {
      ...
      if (message.type === 'metrics') {
        ...
        renderMetrics(message.errors || []);
      }

      if (message.type === 'error') {
        document.getElementById('loading').style.display = 'none';
        document.getElementById('content').style.display = 'block';
        document.getElementById('metricsErrors').innerHTML =
          '<div class="error">Failed to load metrics: ' + (message.message || 'Unknown error') + '</div>';
      }
    });

    function renderMetrics(errors) {
      document.getElementById('loading').style.display = 'none';
      document.getElementById('content').style.display = 'block';
      ...
      const metricsErrorsDiv = document.getElementById('metricsErrors');
      metricsErrorsDiv.innerHTML = errors.length
        ? errors.map(err => '<div class="warning">⚠️ ' + err + '</div>').join('')
        : '';
