...
<head>
  ...
  <style>
    ...
    .result { margin-top: 18px; padding: 14px; background: var(--vscode-textBlockBackground); border-radius: 6px; }
...
<body>
  ...
  <script>
    ...
    function renderResult(response) {
      ...
      metricsDiv.innerHTML = \`
        <div class="metric">
          <div class="metric-value">\${score === undefined || score === null ? 'N/A' : score.toFixed(0) + '%'}</div>
  ...
  private async handleGenerate(request: {
    ...
    try {
      ...
      const parsedFiles =
        response.data?.files?.length ? response.data.files : this.multiFileParser.parse(response.data?.code || '');

      const files = parsedFiles.length
        ? parsedFiles
        : [{
            packageName: '',
      ...
      logInfo(`Code generated: ${files.length} file(s)`);
    } catch (error: any) {
