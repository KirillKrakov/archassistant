    ...
    override fun generate(request: CodeGenerationRequest): CodeGenerationResponse {
        ...
        while (iteration < request.maxIterations) {
            ...
            if (lastViolations.isEmpty() && (lastScore?.total ?: 0.0) >= warningThreshold) {
                if (iteration > 1) {
                    warnings += "Hybrid-Strategy required $iteration/${request.maxIterations} iterations."
                }
                return success(
                    code = generatedCode,
                    score = lastScore,
        ...
        warnings += "Hybrid-Strategy exhausted ${request.maxIterations} iterations. Manual review is recommended."
