...
object ErrorFormatter {
    ...
    fun formatErrorsForPrompt(violations: List<Violation>, maxErrors: Int = 5): String {
        ...
        return limited.joinToString("\n") { violation ->
            when (violation.severity) {
                com.example.archassistant.model.Severity.CRITICAL -> "[КРИТИЧНО] "
                com.example.archassistant.model.Severity.ERROR -> "[ОШИБКА] "
                com.example.archassistant.model.Severity.WARNING -> "[ПРЕДУПРЕЖДЕНИЕ] "
                else -> ""
    ...
    fun formatFixInstruction(violations: List<Violation>, maxErrors: Int = 5): String {
        ...
        val critical = violations.filter { it.severity == com.example.archassistant.model.Severity.CRITICAL }
        val errors = violations.filter { it.severity == com.example.archassistant.model.Severity.ERROR }
