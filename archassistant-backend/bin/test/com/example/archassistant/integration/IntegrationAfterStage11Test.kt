...
class IntegrationAfterStage11Test {
    ...
    private lateinit var hybridStrategy: HybridGenerationStrategy
     ...
     * Захватывает как system, так и user промпты.
     */
    ...
    // region Тесты ComplianceScoreCalculator (такие же как в Stage10, для уверенности)
    @Test
    ...
    // region Тесты Hybrid-стратегии
    @Test
    fun `hybrid-strategy succeeds on first iteration for valid code with rules in system prompt`() {
        val config = RulesConfig(
            projectId = "hybrid.valid",
            rules = listOf(
                ArchitecturalRule(
                    id = "suffix", name = "Service suffix", type = RuleType.NAMING_CONVENTION,
                    fromPackage = "..service..", constraint = ConstraintType.NAMING_SUFFIX,
        ...
        val response = hybridStrategy.generate(request)
        ...
        // Проверяем, что system prompt содержит правила
        val capturedSystemPrompt = systemPromptCaptor.value
        assertTrue(capturedSystemPrompt.contains("Service suffix"))
        assertTrue(capturedSystemPrompt.contains("АРХИТЕКТУРНЫЕ ПРАВИЛА"))
    }

    @Test
    fun `hybrid-strategy stops when score meets threshold on first iteration`() {
        val config = RulesConfig(projectId = "hybrid.threshold", rules = emptyList())
        ruleRepository.save(config)
    ...
    @Test
    fun `hybrid-strategy continues when score is below threshold`() {
        val config = RulesConfig(projectId = "hybrid.below", rules = emptyList())
        ruleRepository.save(config)
    ...
    @Test
    fun `hybrid-strategy retries and succeeds on second iteration with error feedback`() {
        val config = RulesConfig(
            projectId = "hybrid.retry",
            rules = listOf(
                ArchitecturalRule(
                    id = "suffix", name = "Service suffix", type = RuleType.NAMING_CONVENTION,
                    fromPackage = "..service..", constraint = ConstraintType.NAMING_SUFFIX,
        ...
        val lowScore = ComplianceScore(
            ...
            violations = listOf(Violation("suffix", "should end with Service", "UserLogic", severity = Severity.ERROR))
        )
        ...
        assertTrue(allUserPrompts.size >= 2)
        val secondPrompt = allUserPrompts[1]
        assertTrue(secondPrompt.contains("should end with Service"))
        assertTrue(secondPrompt.contains("Исправь эти нарушения"))
    }

    @Test
    fun `hybrid-strategy returns best result after exhausting iterations`() {
        val config = RulesConfig(
            projectId = "hybrid.exhaust",
            rules = listOf(
                ArchitecturalRule(
                    id = "suffix", name = "Service suffix", type = RuleType.NAMING_CONVENTION,
                    fromPackage = "..service..", constraint = ConstraintType.NAMING_SUFFIX,
    ...
    @Test
    fun `hybrid-strategy handles validation exception gracefully`() {
        val config = RulesConfig(projectId = "hybrid.exception", rules = emptyList())
        ruleRepository.save(config)
    ...
    @Test
    fun `hybrid-strategy accumulates generation and validation times across iterations`() {
        val config = RulesConfig(projectId = "hybrid.timing", rules = emptyList())
        ruleRepository.save(config)
        ...
        assertTrue(response.success)
        assertTrue(response.metadata.generationTimeMs >= 90)  // 2 * 50ms - допуск
        assertTrue(response.metadata.validationTimeMs >= 30)  // 2 * 20ms - допуск
        assertEquals(
    ...
    @Test
    fun `hybrid-strategy respects maxIterations and does not exceed`() {
        val config = RulesConfig(projectId = "hybrid.maxiter", rules = emptyList())
        ruleRepository.save(config)
    ...
    @Test
    fun `hybrid-strategy skips validation when class name extraction fails`() {
        val config = RulesConfig(projectId = "hybrid.skip", rules = emptyList())
        ruleRepository.save(config)
    ...
    @Test
    fun `hybrid-strategy handles empty prompt with error`() {
        val response = hybridStrategy.generate(CodeGenerationRequest(prompt = "", projectId = "any"))
        assertFalse(response.success)
